...
> 不定冠詞 「A/An」 除了用來指單數的名詞，也可以用來泛指一件事物。    
> 定冠詞 The 除了用來指稱前面提到的某樣東西。

例如：

```
He is a boy. 他是一個男孩。
He is the boy. 他是那個男孩。(你我所知的，或曾經提到過的某一個男孩)
```

> 注意，`an`是跟`發音`有關，跟字母無關。    
> - 例如：`a unit`，`u`雖然是母音字母，但`unit`的一個始發音不是母音. 
> - 反之，有些字不是母音開始，但卻必須用`an`，例如：`an hour`, `an honest person`
...
最後，還有一種常見的狀況是用在`姓氏`上，若想指稱的是某某一家人，例如王家人，我們會說 the Wang family，那也可以省略 family（家人）變成 the Wang`s`。(指全家人，所以姓氏`Wang`的後面要加`s`)
