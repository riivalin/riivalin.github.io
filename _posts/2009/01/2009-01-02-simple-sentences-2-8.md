...
這時候，補語位置的`Chairman` 雖然仍舊和主詞位置的專有名詞`Bill Gates` 全等、似乎可以視同專有名詞看待，但是在這個上下文中Chairman 已經有兩位了，所以不再符合專有名詞「只有一個」的特性，因此不能把Chairman 視為專有名詞，而必須加限定詞，成為`a better Chairman` 這個「普通名詞片語」。      
