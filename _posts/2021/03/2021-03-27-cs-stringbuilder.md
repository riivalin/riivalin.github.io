...
> `StringBuilder`會維護一個緩衝區，以容納新資料。       
> `string` 是參考型別，所以每次在動態處理資料時，都會再配置一個新的記憶體給它，但是原本使用的資源雖然最終交由 GC(Garbage Collection) 回收，但是在回收之前資源卻已經浪費掉了。       
>
> [C# 中 String 直接累加與 StringBuilder](https://shunnien.github.io/2017/08/30/string-vs-stringBuilder-in-csharp/)
...
- `Boxing`(裝箱)：
將堆疊(`stack`)中的值複製到堆積(heap)中的行為，就叫 Boxing(裝箱)。

- `Unboxing`(拆箱)：
反過來說，將堆積(`heap`)中的值，複製到堆疊(stack)中的行為，就叫 Unboxing(拆箱)。
